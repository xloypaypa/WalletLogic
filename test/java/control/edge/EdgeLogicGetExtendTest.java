 ...
 * it's the testing code for edge logic get extend
 */
public class EdgeLogicGetExtendTest extends LogicTesting {
    ...
    public void checkUrl() throws Exception {
        ...
        EdgeLogicNoSend edgeLogic = getEdgeExtend(this.socket, "a");

        assertEquals("/getExtendEdgeList", edgeLogic.getEvent().getMessage().get(0).getKey());
    }
    ...
    public void shouldSendDataWhenGetData() throws Exception {
        ...
        createNewBudget(this.socket, "b", "1", "2");
        addEdge(this.socket, "a", "b", "c");
        EdgeLogicNoSend edgeLogic = getEdgeExtend(this.socket, "b");
